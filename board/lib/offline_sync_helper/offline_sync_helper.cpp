...
bool OfflineSyncHelper::syncPendingEvents(bool (*sendCallback)(const char*, const char*))
{
  ...
  int successCount = 0;
  int failCount = 0;
  const int BATCH_LIMIT = 10;

  for (JsonVariant event : events)
  {
    if (successCount + failCount >= BATCH_LIMIT)
    {
      remaining.add(event);
      continue;
    }
  ...
  Serial.printf("[Sync] Sync complete: %d success, %d failed, %d deferred\n", successCount, failCount,
                remaining.size());
...
void OfflineSyncHelper::attemptOfflineSync(OfflineSyncHelper& offlineSync, unsigned long& lastSyncAttempt,
                                           const unsigned long SYNC_INTERVAL)
{
  if (!MQTT::isConnected())
  {
    return;
  ...
  if (!offlineSync.hasPendingEvents())
  {
    return;
  }
